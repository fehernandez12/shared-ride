...
def generate_code(k):
    "Generate a random k-digits passcode."
    return ''.join(random.choices(string.ascii_uppercase + string.digits, k=k))


def generate_otp(user):
    "Create OTP passcode that the user can use to verify their account."
    otp = OTP(
        user=user
    )
    while True:
...
def send_confirmation_email(user_pk):
    ...
    verification_code = generate_otp(user)
    ...
    content = render_to_string(
        ...
        {'token': verification_code, 'user': user}
    )
